...
redis_server_streaming_df = spark\
    .readStream\
    .format("kafka")\
...
casted_redis_streaming_df = redis_server_streaming_df\
...
casted_redis_streaming_df\
.withmColumn("value", from_json("value", redis_message_schema))\
...
zSetEntriesDecodedStreamingDF = zSetEntriesEncodedStreamingDF.withColumn("customer", unbase64(col("encodedCustomer")).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
zSetEntriesDecodedStreamingDF.withColumn("customer", from_json("customer", customer_schema))\
.select(col('customer.*'))\
...
result_df = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("customer = email"))
